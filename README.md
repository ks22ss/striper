...
Paste a prompt, optionally add sample input text the prompt will process, optionally enter an OpenAI API key (uses server config if blank), click **Analyze**, and view the over-engineered score, improved prompt, and component breakdown (kept vs removed). From **History**, use the **Copy** button on any item to copy its improved prompt to the clipboard.
