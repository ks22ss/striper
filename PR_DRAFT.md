...
### Solution
- **Handle `AuthenticationError`** – Added `except AuthenticationError` before the generic `Exception` in the analyze endpoint. Returns 503 with "Invalid OpenAI API key" for better UX.
...
| File | Changes |
|------|---------|
| `app/main.py` | Import `AuthenticationError`; add `except AuthenticationError` → 503 |
| `app/stripe.py` | `DEFAULT_SIMILARITY_THRESHOLD` constant; use in `_get_similarity_threshold` |
| `tests/test_main.py` | `test_analyze_invalid_api_key_returns_503` regression test |
