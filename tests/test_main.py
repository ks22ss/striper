...
@pytest.mark.parametrize(
    "id_suffix,expected_strings",
    [
        ("prompt_length", ["prompt-count", "chars", "words"]),
        ("copy_report", ["copy-report-btn", "Copy report"]),
        ("theme_toggle", ["theme-toggle", "data-theme", "system"]),
        ("use_improved", ["use-improved-btn", "Use as prompt"]),
        ("download_json", ["download-json-btn", "Download JSON"]),
        ("input_length", ["input-count"]),
        ("clear_form", ["clear-form-btn", "Clear"]),
        ("landing", ["landing-page", "Get started", "Login", "Register"]),
        ("login_page", ["login-page", "login-form", "login-submit-btn", "#/register"]),
        ("register_page", ["register-page", "register-form", "register-submit-btn", "#/login"]),
        ("app_page", ["app-page", "logout-btn", "Logout"]),
        ("history_shortcut", ["history-btn", "Ctrl+Shift+H"]),
        ("escape_close", ["history-back", "Esc"]),
    ],
    ids=lambda x: x[0] if isinstance(x, tuple) else x,
)
def test_ui_includes_elements(id_suffix, expected_strings):
