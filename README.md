...
Paste a prompt (character count shown live), optionally add sample input text the prompt will process, optionally enter an OpenAI API key (uses server config if blank), click **Analyze**, and view the over-engineered score, improved prompt, and component breakdown (kept vs removed).
