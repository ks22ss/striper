...
Paste a prompt, optionally add sample input text the prompt will process, optionally enter an OpenAI API key (uses server config if blank), click **Analyze** (or press **Ctrl+Enter** / **Cmd+Enter**), and view the over-engineered score, improved prompt, and component breakdown (kept vs removed). Use the **Copy** button next to the improved prompt to copy it to the clipboard. Use **Use as prompt** to load the improved prompt into the form for re-analysis or further editing. In **History**, click any item to load its prompt into the form and re-analyze.
